<?php
  ...
  if ($level == "receptionist") {
    $queryReserve = mysqli_query($koneksi, "SELECT * FROM pesan_table JOIN users ON pesan_table.tamu_id=users.tamu_id ORDER BY pesan_table.tanggal_pesan DESC");
  } else {
    $queryReserve = mysqli_query($koneksi, "SELECT * FROM pesan_table JOIN users ON pesan_table.tamu_id=users.tamu_id WHERE pesan_table.tamu_id='$guest_id' ORDER BY pesan_table.tanggal_pesan DESC");
  }
